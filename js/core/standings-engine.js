...
// مشارك "لسه ما دخل فعليًا الموسم كـ(اعتبارًا من رقم جولة معيّن)" — انضم بعد
// جولات حقيقية فعلية (لا فقط بعد فترة الجسر 1-2) ولم تُكمَل بعد الجولة اللي
// يبدأ منها. يُستخدم لإخفائه بالكامل من كل قوائم/جداول الترتيب والإحصائيات
// والميداليات والألقاب حتى تبدأ جولته الأولى فعليًا، حتى لا يؤثر رصيده
// الافتتاحي على أرقام مراكز بقية المشاركين أو نتائجهم بالجولات اللي لم
// يشارك فيها (طلب المستخدم صراحة عند إضافة لطفي (id:20) — 19 سبتمبر 2026؛
// راجع ملف التسليم القسم 9.57).
// ملاحظة: لا يؤثر هذا على أبو صالح/محمد عثمان (كلاهما تجاوز جولته الأولى
// فعليًا منذ زمن)، فيبقيان ظاهرين بلا أي تغيير.
//
// مهم جدًا: الوسيط `asOfRoundNumber` لازم يكون رقم الجولة اللي *يُحسب لها*
// الترتيب بهذا الاستدعاء تحديدًا (مثلاً بحساب ترتيب تاريخي "حتى الجولة 2")،
// وليس دائمًا رقم الجولة الحالية الفعلية للموقع كاملة. الاستدعاء بدون هذا
// الوسيط يرجع للسلوك الافتراضي (الجولة الحالية الفعلية) — يُستخدم لأي مكان
// آخر يحتاج يسأل "هل هذا المشارك نشط الآن فعليًا؟" بمعزل عن أي حساب تاريخي.
//
// خطأ اكتُشف وأُصلح 21 سبتمبر 2026 (القسم 9.59 بملف التسليم): كانت هذي
// الدالة تتحقق دائمًا من getCurrentRoundNumber() (رقم الجولة الحالية الفعلية
// للموقع) حتى لو الاستدعاء كان لحساب ترتيب تاريخي لجولة سابقة (مثل
// computeStandings(1) أو computeStandings(2) المستخدمة بـ"تاريخ الصدارة"
// و"لوحة قيادة الترتيب/السباق" بـstandings-widgets.js) — فبمجرد ما صارت
// الجولة الحالية الفعلية 5 (تجاوزت جولة انضمام لطفي=4)، صار لطفي "مؤهّل"
// حتى بحسابات تاريخية لجولات 1-4 اللي هو أصلًا ما شارك فيها، فظهر بتاريخ
// إنجازات ما حقّقها فعليًا.
function isParticipantActiveNow(pid, asOfRoundNumber){
  const roundToCheck = (asOfRoundNumber === undefined) ? getCurrentRoundNumber() : asOfRoundNumber;
  return roundToCheck > (JOINED_AFTER_ROUND[pid]||0);
}
...
function computeStandings(upTo){
  ...
  // رقم الجولة "كما لو كنا فيها" لهذا الاستدعاء تحديدًا — يعتمد على آخر جولة
  // ضمن usedRounds (نافذة هذا الاستدعاء)، وليس دائمًا على DATA.rounds كاملة.
  // هذا ما يخلي إخفاء لطفي (وأي منضمّ لاحقًا بنفس الآلية) يعمل صح بكل حساب
  // تاريخي لجولة سابقة (تاريخ الصدارة، لوحة قيادة الترتيب...)، لا بس بالترتيب
  // الحالي العام (طلب المستخدم 21 سبتمبر 2026 — القسم 9.59).
  const asOfRoundNumber = usedRounds.length ? usedRounds[usedRounds.length-1].number : CURRENT_BASE_ROUND;
  ...
  const eligibleList = list.filter(s=> isParticipantActiveNow(s.id, asOfRoundNumber));
