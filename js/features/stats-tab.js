...
function renderClubMap(){
  ...
  // ترتيب الدوريات: قائمة مفضّلة معروفة أولًا، ثم أي دوري جديد ظهر بـ
  // CLUB_LEAGUE_MAP (مثلاً لنادٍ جاء مع مشارك جديد) يُضاف تلقائيًا بالترتيب
  // الأبجدي بعدها، بدل ما يختفي بصمت لأنه غير مذكور بقائمة ثابتة. هذا يمنع
  // تكرار مشكلة "شختار دونيتسك/الدوري الأوكراني ما ظهر بخريطة الأندية" مستقبلاً
  // مع أي نادٍ/دوري جديد (طلب المستخدم 19 سبتمبر 2026 — لا حاجة لتذكّر إضافة
  // كل قائمة يدويًا عند كل مشارك جديد).
  const preferredLeagueOrder = ['الدوري السعودي','الدوري الإسباني','الدوري الإنجليزي','الدوري الإيطالي','الدوري الألماني','الدوري الفرنسي','الدوري البرتغالي','الدوري الهولندي','الدوري التركي','الدوري الأسكتلندي','الدوري الكرواتي','الدوري الأوكراني'];
  const leagueFlags = {'الدوري السعودي':'🇸🇦','الدوري الإسباني':'🇪🇸','الدوري الإنجليزي':'🏴󠁧󠁢󠁥󠁮󠁧󠁿','الدوري الإيطالي':'🇮🇹','الدوري الألماني':'🇩🇪','الدوري الفرنسي':'🇫🇷','الدوري البرتغالي':'🇵🇹','الدوري الهولندي':'🇳🇱','الدوري التركي':'🇹🇷','الدوري الأسكتلندي':'🏴󠁧󠁢󠁳󠁣󠁴󠁿','الدوري الكرواتي':'🇭🇷','الدوري الأوكراني':'🇺🇦'};
  const extraLeagues = Object.keys(byLeague)
    .filter(l=> l!=='أخرى' && !preferredLeagueOrder.includes(l))
    .sort((a,b)=>a.localeCompare(b,'ar'));
  const leagueOrder = [...preferredLeagueOrder, ...extraLeagues, 'أخرى'];
